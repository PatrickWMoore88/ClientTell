...
router.get('/get/team_members', async (req, res) => {
...
router.post('/create/team_members', async (req, res) => {
...
router.post('/delete/team_members/:id', async (req, res) => {
...
router.get('/update/team_members/:id', async (req, res) => {
  try {
    const result = await db.runQuery(`SELECT * FROM team_members WHERE ID = $1`, [req.params.id]);
...
router.post('/update/team_members/:id', async (req, res) => {
  ...
  res.render('getTeamMembers', { title: 'Team Members', teamMember: result.rows[0]});
});
